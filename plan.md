...
- Fixed false missing-value previews at the shared lazy-choice boundary: lazy reference cells now preserve and display their raw value until the target options are materialized, while fully materialized editors still mark only truly absent references as missing. Applied the same authoritative-options rule to lazy enum previews, filter-modal enum/reference controls, enum filter matching, and filter-summary labels; regression coverage and the full 109-pass suite are green.
